...
4. After installation, remind them to check in **Shopify Admin → Settings → Payments** that the new provider shows as active.
   **Test before going live:** the Paymob Shopify app has a **test mode** button in its settings. Have the merchant turn it on, place a storefront order paid with the sandbox test details for their enabled methods (see `testing-guide.md` — card numbers come only from `test-credentials.md`), confirm the order shows as paid in Shopify admin and the transaction appears in the Paymob dashboard, then **turn test mode off** before real customers check out.
